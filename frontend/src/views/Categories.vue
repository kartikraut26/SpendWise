<template>
  <div class="module-page">
    <section class="page-heading">
      <div>
        <p class="eyebrow">ORGANISE</p>
        <h1>Categories</h1>
        <p>Create categories that match the way you manage money.</p>
      </div>
    ...
    <div class="columns">
      <section class="glass-surface card">
        <div class="card-head">
          <h2>Expense categories</h2>
          <span>{{ expenses.length }}</span>
        </div>
        <div v-for="c in expenses" :key="c._id" class="row">
      ...
      <section class="glass-surface card">
        ...
        <div v-for="c in incomes" :key="c._id" class="row">
          <div class="swatch" :style="{ background: c.color }"></div>
          <div class="name">
            <strong>{{ c.name }}</strong
            ><small>{{ c.type }}</small>
    ...
    <div v-if="modalOpen" class="backdrop" @click.self="close">
      <form class="modal glass-surface" @submit.prevent="save">
        ...
        <p v-if="form.type === 'expense'" class="hint">
          Existing transactions using a renamed category will be updated
          automatically.
        </p>
...
const incomes = computed(() =>
  finance.categories.filter((c) => c.type === "income"),
);
async function load() {
  try {
    await finance.fetchCategories();
  } catch (e) {
    error.value = e.response?.data?.message || "Could not load categories.";
  }
}
function openCreate() {
  editing.value = null;
  Object.assign(form, { name: "", type: "expense", color: "#6366f1" });
...
async function save() {
  try {
    if (editing.value) await api.patch(`/categories/${editing.value}`, form);
...
async function remove(c) {
  if (!confirm(`Delete "${c.name}"? Transactions will become uncategorized.`))
    return;
  try {
    await api.delete(`/categories/${c._id}`);
    await load();
  } catch (e) {
    error.value = e.response?.data?.message || "Could not delete category.";
  }
}
onMounted(load);
</script>
